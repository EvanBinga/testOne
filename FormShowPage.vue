    ...
    <main class="flex items-center justify-center h-screen">
        ...
        ">
            <div v-if="feedback.isLoaded" class="flex flex-col">
                ...
                <!-- Отображение выбранного сервиса и рейтинга -->
                <p class="text-base mt-2 text-gray-500">Выбранный сервис: {{ feedback.service }}</p>
                <p class="text-base mt-2 text-gray-500">Рейтинг: {{ parseFloat(feedback.rating) }}</p>
            </div>
            <div>
                <RouterLink
...
const feedback = reactive({
    ...
    datetime: '',
    service: '',
    rating: '', // Добавленное поле для рейтинга
    isLoaded: false
});
...
onBeforeMount(() => {
    axios.get<FeedbackResponse>(env.backend_url + `/feedbacks/${idFromRouter}`)
        .then(feedbackResponse => {
            ...
            feedback.service = feedbackResponseData.service; // Добавленная строка для получения сервиса
            feedback.rating = feedbackResponseData.rating; // Добавленная строка для получения рейтинга
...
interface FeedbackResponse {
    ...
    description: string;
    datetime: string;
    service: string; // Добавленное поле для сервиса
    rating: string; // Добавленное поле для рейтинга
}
</script>
